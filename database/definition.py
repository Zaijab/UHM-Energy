...
Campus <- BuildingComplex <- Building <- Meter (Key_Perf <- Main_Meter <- Sub_Meter)
...
class Campus(Base):
    ...
    campus_id = Column(String(255), primary_key=True)
...
class Building(Base):
    ...
    building_id = Column(String(255), primary_key=True)
    building_complex = Column(String(255), nullable=False)
    path = Column(LtreeType, nullable=False)
    parent = relationship(
        'Meter',
        primaryjoin=remote(path) == foreign(func.subpath(path, 0, -1)),
    ...
    def __repr__(self):
        return 'Building({})'.format(self.building_id)
